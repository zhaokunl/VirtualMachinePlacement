...
public class Host {
	...
	public void addMigratingInVm(Vm vm) {
		...
		if (!getVmsMigratingIn().contains(vm)) {
			...
			if (!getBwProvisioner().allocateBwForVm(vm,
			...
			if (!getVmScheduler().allocatePesForVm(vm,
	...
	public void reallocateMigratingInVms() {
		for (Vm vm : getVmsMigratingIn()) {
			...
			getRamProvisioner().allocateRamForVm(vm,
					vm.getCurrentRequestedRam());
	...
	public boolean isSuitableForVm(Vm vm) {
		return (getVmScheduler().getPeCapacity() >= vm
				...
				&& getRamProvisioner().isSuitableForVm(vm,
						vm.getCurrentRequestedRam()) && getBwProvisioner()
	...
	// rAverage = average utilization of all resources of a server
	// n = number of resources
	// p = is the server p
	public double skewnessOfServer() {

		// current resource utilization, only bottleneck resources are
		// considered
		double skewness;
		double currentRAMutilization = (getRamProvisioner().getUsedRam())
				/ (getRamProvisioner().getRam());
		double currentBWutilization = (getBwProvisioner().getUsedBw())
				/ (getBwProvisioner().getBw());
	...
	// FUSD alg needs to calculate temperature of each server
	// this alg will be applied every active server(host) in system
	// rThreshold = hot theshold
	// R = set of overloaded resources
	// r = current utilization resource r
	// p = is the server p
	public double hotTemperatureOfServer() {
		...
		// definitions for threshold values for resources (as stated in
		// corresponding paper)
		...
		if (currentRAMutilization >= hotThreshold) {
			temperatureHot = temperatureHot
					+ Math.pow((currentRAMutilization - hotThreshold), 2);
		}
		if (currentBWutilization >= hotThreshold) {
			temperatureHot = temperatureHot
					+ Math.pow((currentBWutilization - hotThreshold), 2);
		}
		if (currentMIPSutilization >= hotThreshold) {
			temperatureHot = temperatureHot
					+ Math.pow((currentMIPSutilization - hotThreshold), 2);
		}
	...
	public double coldTemperatureOfServer() {
		...
		if (currentRAMutilization <= coldThreshold) {
			temperatureCold = temperatureCold
					+ Math.pow((currentRAMutilization - coldThreshold), 2);
			if (currentBWutilization <= coldThreshold) {
				temperatureCold = temperatureCold
						+ Math.pow((currentBWutilization - coldThreshold), 2);
				if (currentMIPSutilization <= coldThreshold) {
					temperatureCold = temperatureCold
							+ Math.pow(
									(currentMIPSutilization - coldThreshold), 2);
				}
			}
		}
	...
	public double greenComputingTemperatureOfServer() {
		...
		if (averageUtilization <= greenComputingThreshold) {
			temperatureGreen = Math.pow(
					(averageUtilization - greenComputingThreshold), 2);
		}
